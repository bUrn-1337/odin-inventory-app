...
async function deleteRarity(rarityName) {
    await pool.query("DELETE FROM rarities WHERE rarity_name = $1;", rarityName);    
}

async function updateRarity(oldRarityName, newRarityName) {
    await pool.query("UPDATE rarities SET rarity_name = $1 WHERE rarity_name = $2; ", newRarityName, oldRarityName);
}
